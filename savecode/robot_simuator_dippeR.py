...
class RobotSimulatorDiPPeR:
    ...
    def dipperp_path_planning(self, start, goal):
        ...
        try:
            ...
            # 정규화된 좌표를 실제 좌표로 변환 (학습 시와 동일한 스케일링)
            path_normalized = torch.clamp(generated_path[0], -1.0, 1.0)  # 안전한 범위로 제한
            path_real = path_normalized.cpu().numpy() * 6.0  # 학습 시와 동일한 스케일링 (-1~1 → -6~6)
    ...
    def is_position_safe(self, pos):
        ...
        # 그리드에서 장애물 체크 (안전성 검증 완화: 1.0 → 0.9)
        if self.grid[y_idx, x_idx] >= 0.9:
            return False
        
        # 추가 안전 마진 체크 완화 (1셀 → 0셀)
        margin_cells = 0  # 마진 제거로 더 관대하게
        for i in range(-margin_cells, margin_cells + 1):
            for j in range(-margin_cells, margin_cells + 1):
                check_x = x_idx + j
                check_y = y_idx + i
                if (0 <= check_x < 60 and 0 <= check_y < 60 and 
                    self.grid[check_y, check_x] >= 0.9):  # 임계값 완화
                    return False
